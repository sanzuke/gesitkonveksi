  ...
  case 'getCategory':
    ...
    case 'auth':
      ...
      case "plugin-wp":
      	...
      	$noresi = mysql_real_escape_string($_POST['noresi']);
      	$data = array();
      	//$result =  mysql_query("SELECT t.*, pv.parametervalue, pv.parametervaluecode, pv.option FROM um_tracking t, ss_parametervalue pv WHERE t.progress = pv.parametervaluecode AND t.noresi='{$noresi}' ");
      	$result = mysql_query("SELECT p.*, pv.parametervalue, pv.option, pv.parametervaluecode
FROM um_progress p, ss_parametervalue pv 
WHERE p.noresi = '{$noresi}' 
AND pv.parametervaluecode = p.progress
